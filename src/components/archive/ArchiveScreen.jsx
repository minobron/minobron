...
// Applica ordine personale da localStorage
function applyLocalOrder(items, storedIds) {
  ...
  const newItems = items.filter(i => !storedIds.includes(i.id))
...
export default function ArchiveScreen() {
  ...
  const [reorderFolders, setReorderFolders]     = useState([])
  ...
  // localStorage key per ordine cartelle
  const folderOrderKey = user && wsId ? `mino_folder_order_${user.uid}_${wsId}` : null
  ...
  useEffect(() => {
    ...
    if (!isPdf) return
    let blobUrl = null; let cancelled = false
    ...
    fetch(preview.url)
      ...
      .then(blob => {
        ...
        setPdfBlobUrl(blobUrl); setPdfLoading(false)
      })
  ...
  useEffect(() => {
    ...
    setFolder(null); setFolders([]); setShowTrash(false); setFileSearch('')
  }, [wsId])
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      if (snap.empty && !seededRef.current) {
        ...
        // Applica ordine personale
        const storedIds = (() => {
          try { return JSON.parse(localStorage.getItem(folderOrderKey)) || [] }
          catch { return [] }
        })()
        const ordered = applyLocalOrder(raw, storedIds)
        setFolders(ordered)
        setFolder(f => f || ordered[0]?.name)
      }
    })
  }, [wsId, folderOrderKey])
  ...
  // Ricerca file cross-cartella
  useEffect(() => {
    if (!wsId || fileSearch.trim().length < 2) {
      setSearchResults([]); return
    }
    setSearchLoading(true)
    const lower = fileSearch.toLowerCase()
    const q = collection(db, `workspaces/${wsId}/files`)
    const unsub = onSnapshot(q, snap => {
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      setSearchResults(all.filter(f => f.name?.toLowerCase().includes(lower)))
      setSearchLoading(false)
    })
    return unsub
  }, [wsId, fileSearch])

  // Carica cestino + auto-pulizia
  useEffect(() => {
    if (!wsId) return
    ...
    return onSnapshot(q, async snap => {
      ...
      const toDelete = list.filter(f => {
        const d = f.deletedAt?.toDate ? f.deletedAt.toDate() : new Date(f.deletedAt)
        return differenceInDays(now, d) >= TRASH_DAYS
      })
  ...
  const openReorderSheet = () => {
    setReorderFolders([...folders])
    setShowReorderSheet(true)
  }

  const saveReorderFolders = () => {
    setFolders(reorderFolders)
    if (folderOrderKey) {
      localStorage.setItem(folderOrderKey, JSON.stringify(reorderFolders.map(f => f.id)))
    }
  ...
  const saveRenameFolder = async () => {
    ...
    const snap = await getDocs(query(collection(db, `workspaces/${wsId}/files`), where('folder', '==', oldName)))
    await Promise.all(snap.docs.map(d => updateDoc(doc(db, `workspaces/${wsId}/files/${d.id}`), { folder: newName })))
    if (folder === oldName) setFolder(newName)
    setRenamingFolder(null); setFolderSheet(null)
  }
  ...
  const doDeleteFolder = async () => {
    ...
    if (fallback) {
      const snap = await getDocs(query(collection(db, `workspaces/${wsId}/files`), where('folder', '==', folderName)))
      await Promise.all(snap.docs.map(d => updateDoc(doc(db, `workspaces/${wsId}/files/${d.id}`), { folder: fallback })))
    }
    await deleteDoc(doc(db, `workspaces/${wsId}/folders/${folderId}`))
    if (folder === folderName) setFolder(fallback || null)
    setFolderSheet(null); setConfirmDeleteFolder(null)
  }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      await addDoc(collection(db, `workspaces/${wsId}/files`), {
        name, url, type, folder, size, uploadedBy: user.uid, createdAt: serverTimestamp()
      })
  ...
  const deleteFile = async (file) => {
    await addDoc(collection(db, `workspaces/${wsId}/trash`), {
      ...file, originalFolder: file.folder || folder,
      deletedAt: serverTimestamp(), deletedBy: user.uid, deletedByName: user.name || user.email,
    })
    await deleteDoc(doc(db, `workspaces/${wsId}/files/${file.id}`))
    setPreview(null); setFileSheet(null); setConfirmDeleteFile(null)
  }

  const restoreFile = async (trashFile) => {
    const { id, deletedAt, deletedBy, deletedByName, originalFolder, ...fileData } = trashFile
    await addDoc(collection(db, `workspaces/${wsId}/files`), {
      ...fileData, folder: originalFolder || folders[0]?.name || 'Generale', restoredAt: serverTimestamp(),
    })
  ...
  const saveRenameFile = async () => {
    ...
    await updateDoc(doc(db, `workspaces/${wsId}/files/${renamingFile.id}`), { name: renamingFile.name.trim() })
    if (preview?.id === renamingFile.id) setPreview(p => ({ ...p, name: renamingFile.name.trim() }))
    setRenamingFile(null); setFileSheet(null)
  }
  ...
  const isSearching = fileSearch.trim().length >= 2
  ...
  return (
    <div className="max-w-lg mx-auto pb-4">
      ...
      {/* ── Tab cartelle + cestino ────────────────────────────────────────── */}
      <div className="sticky top-0 z-10 px-4 pt-3 pb-2" style={{ background: 'var(--c-bg)' }}>
        {/* Riga cartelle + pulsanti — badge cestino fuori dall'overflow */}
        <div className="flex items-center gap-2">
          {/* Scroll orizzontale cartelle */}
          <div className="flex gap-2 overflow-x-auto scrollbar-hide flex-1 items-center">
            ...
            {/* Pulsante riordina cartelle */}
            {!showTrash && (
              <button onClick={openReorderSheet}
                className="w-7 h-7 flex items-center justify-center rounded-full flex-shrink-0"
                style={{ background: 'rgba(255,255,255,0.06)', color: '#6b7280' }}
                title="Riordina cartelle">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Pulsante Cestino — FUORI dall'overflow per non tagliare il badge */}
          <button onClick={() => setShowTrash(s => !s)}
            className="relative w-7 h-7 flex items-center justify-center rounded-full flex-shrink-0"
        ...
        {/* Barra di ricerca file (solo fuori dal cestino) */}
        {!showTrash && (
          <div className="relative mt-2">
            ...
            <input value={fileSearch} onChange={e => setFileSearch(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
            ...
            <>
              <div className="flex items-center justify-between px-1">
                <p className="text-xs text-gray-600">
                  {trashFiles.length} file — eliminati automaticamente dopo {TRASH_DAYS} giorni
                </p>
                <button onClick={() => setConfirmEmptyTrash(true)}
                  className="text-xs text-rose-400 font-medium">Svuota</button>
              </div>
                  ...
                  <motion.div key={file.id}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-600 mt-0.5">
                        {file.deletedByName || '—'} · {days}g rimasti
                      </p>
                    </div>
        ...
        <div className="px-4 space-y-3">
          {searchLoading ? (
            ...
            <div className="flex flex-col items-center py-12 gap-2">
              ...
              <p className="text-sm text-gray-700">Nessun file trovato per "{fileSearch}"</p>
            </div>
          ) : (
            <>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md"
                        ...
                        {file.folder || 'Generale'}
                      </span>
                      {file.createdAt && (
                        <span className="text-[10px] text-gray-600">
                          {format(file.createdAt.toDate(), 'd MMM', { locale: it })}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {preview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex items-center gap-3 px-4 flex-shrink-0"
              style={{ paddingTop: 'max(0.75rem, env(safe-area-inset-top))', paddingBottom: '0.75rem', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(16px)' }}
            ...
            <div className="flex-1 flex items-center justify-center p-4 overflow-hidden">
              {(() => {
                if (preview.type?.startsWith('image/'))
                  return <img src={preview.url} alt={preview.name} onClick={e => e.stopPropagation()} className="max-w-full max-h-full object-contain rounded-xl" />
                if (preview.type?.includes('pdf') || preview.name?.match(/\.pdf$/i)) {
                  if (pdfLoading) return <div className="flex flex-col items-center gap-3" onClick={e => e.stopPropagation()}><div className="w-8 h-8 border-2 border-primary-400 border-t-transparent rounded-full animate-spin" /><p className="text-xs" style={{ color: '#6b7280' }}>Caricamento PDF…</p></div>
                  if (pdfError || !pdfBlobUrl) return <div className="flex flex-col items-center gap-4" onClick={e => e.stopPropagation()}><span className="text-6xl">📄</span><p className="text-sm text-center px-4" style={{ color: '#9ca3af' }}>{preview.name}</p><a href={preview.url} target="_blank" rel="noreferrer" className="px-6 py-3 rounded-2xl text-sm font-semibold" style={{ background: '#6366f1', color: 'white' }}>Apri PDF</a></div>
                  return <iframe src={pdfBlobUrl} title={preview.name} onClick={e => e.stopPropagation()} className="w-full h-full rounded-xl border-0" />
                }
                const iframeUrl = getPreviewUrl(preview)
                if (iframeUrl) return <iframe src={iframeUrl} title={preview.name} onClick={e => e.stopPropagation()} className="w-full h-full rounded-xl border-0" sandbox="allow-scripts allow-same-origin allow-popups" />
                return <div className="flex flex-col items-center gap-4" onClick={e => e.stopPropagation()}><span className="text-6xl">{getFileIcon(preview.type)}</span><p className="text-sm text-center" style={{ color: '#9ca3af' }}>{preview.name}</p><p className="text-xs text-center" style={{ color: '#6b7280' }}>Formato non visualizzabile — usa Salva per aprirlo</p></div>
              })()}
            </div>
            <div className="flex gap-3 px-4 flex-shrink-0"
              ...
              <button onClick={() => downloadFile(preview.url, preview.name)}
                ...
                style={{ background: '#6366f1', color: 'white' }}>⬇️ Salva</button>
              <button onClick={() => setConfirmDeleteFile(preview)}
                className="px-4 py-3 rounded-2xl text-sm font-semibold text-rose-400"
                style={{ background: 'rgba(239,68,68,0.1)' }}>🗑️</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ── Sheet riordina cartelle ───────────────────────────────────────── */}
      <AnimatePresence>
        {showReorderSheet && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              style={{ background: 'var(--c-surface2)', paddingBottom: 'max(1.5rem, env(safe-area-inset-bottom))', maxHeight: '80dvh', overflowY: 'auto' }}
              onClick={e => e.stopPropagation()}>
              <div className="w-10 h-1 rounded-full mx-auto" style={{ background: 'rgba(255,255,255,0.1)' }} />
              <h3 className="text-base font-bold text-white">Ordine cartelle</h3>
              <p className="text-xs text-gray-600">Trascina il simbolo ≡ per riordinare. L'ordine è personale.</p>
              <Reorder.Group axis="y" values={reorderFolders} onReorder={setReorderFolders}
                className="space-y-2">
                {reorderFolders.map(f => (
                  <ReorderFolderItem key={f.id} folder={f} active={folder === f.name} />
                ))}
              </Reorder.Group>
              <motion.button whileTap={{ scale: 0.97 }} onClick={saveReorderFolders}
                className="w-full py-3 bg-primary-600 text-white font-semibold rounded-xl text-sm">
                ✓ Salva ordine
              </motion.button>
            </motion.div>
          </motion.div>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              {renamingFolder ? (
                <>
                  ...
                  <input autoFocus value={renamingFolder.name}
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setRenamingFolder(null)}
                      ...
                      style={{ background: 'rgba(255,255,255,0.06)' }}>Annulla</button>
                ...
                <>
                  ...
                  <button onClick={() => setRenamingFolder({ id: folderSheet.id, name: folderSheet.name })}
                    className="w-full text-left py-3 text-sm font-medium text-gray-200">✏️  Rinomina</button>
                  {folders.length > 1 && (
                    <button onClick={() => { setConfirmDeleteFolder(folderSheet); setFolderSheet(null) }}
                      className="w-full text-left py-3 text-sm font-medium text-rose-400">🗑️  Elimina cartella</button>
                  )}
                </>
              )}
            </motion.div>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              {renamingFile ? (
                <>
                  ...
                  <input autoFocus value={renamingFile.name}
                ...
                <>
                  ...
                  <button onClick={() => { setPreview(fileSheet); setFileSheet(null) }}
                    className="w-full text-left py-3 text-sm font-medium text-gray-200">👁️  Apri anteprima</button>
                  <button onClick={() => setRenamingFile({ id: fileSheet.id, name: fileSheet.name })}
                    className="w-full text-left py-3 text-sm font-medium text-gray-200">✏️  Rinomina</button>
                  <button onClick={() => { setConfirmDeleteFile(fileSheet); setFileSheet(null) }}
                    className="w-full text-left py-3 text-sm font-medium text-rose-400">🗑️  Sposta nel cestino</button>
                </>
              )}
            </motion.div>
          </motion.div>
      ...
      <ConfirmDialog
        ...
        message={`Il file sarà recuperabile per ${TRASH_DAYS} giorni, poi verrà eliminato definitivamente.`}
      ...
      <ConfirmDialog
        ...
        message={`${trashFiles.length} file saranno eliminati definitivamente.`}
...
function FolderChip({ folder, active, onTap, onLongPress }) {
  ...
  return (
    <button {...lp}
      ...
      style={{ background: active ? '#6366f1' : 'rgba(255,255,255,0.06)', color: active ? '#fff' : '#6b7280' }}>
      {folder.name}
    </button>
  )
}

function ReorderFolderItem({ folder, active }) {
  const controls = useDragControls()
  return (
    <Reorder.Item value={folder} dragListener={false} dragControls={controls}
      className="flex items-center gap-3 px-4 py-3 rounded-xl select-none"
      style={{ background: active ? 'rgba(99,102,241,0.15)' : 'var(--c-card)', border: '1px solid var(--c-border)', cursor: 'default' }}>
      <div
      ...
      <span className={`text-sm font-medium flex-1 ${active ? 'text-primary-300' : 'text-gray-300'}`}>
        {folder.name}
      </span>
      {active && <span className="text-[10px] text-primary-400 font-semibold">attiva</span>}
    </Reorder.Item>
  )
}
